...
def main():
    ...
    if is_trained:
        st.success("Trained Model Loaded: 77.06% Accuracy")
    else:
        st.warning("Demo Mode: - Using untrained model")
    ...
    with col2:
        if 'prediction_results' in st.session_state:
            ...
            for i, idx in enumerate(top_3_indices):
                ...
                rank = ["1st", "2nd", "3rd"][i]
                st.write(f"{rank} Place: {CLASS_NAMES[idx].title()}: {prob:.1%}")
